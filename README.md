...
```bash
node bin/obsidian-sync.js configure --vault "C:\path\to\vault"
```

The `init`, `plan`, `apply`, and `unify` workflows then sync after saving PAUL files. Notes appear in `20 Projects/PAUL/<project-name>-<id>/` with an Overview note and links to the mirrored project, roadmap, state, plans, and summaries. Sync never removes old notes and refuses to overwrite a note that was not generated by PAUL. Other vault folders and Obsidian settings are untouched.

To refresh manually, run `node bin/obsidian-sync.js sync --project "<project directory>"`. Add `--dry-run` to inspect the target before writing. A project can override the global vault by placing `{ "vault": "<vault path>" }` in `.paul/obsidian.json`.
